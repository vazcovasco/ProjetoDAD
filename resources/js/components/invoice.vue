  ...
  <div>
    ...
    <div>
      <invoice-list
        :invoices="invoices"
        ref="invoicesListRef"
        @show-click="showInvoice"
        @edit-click="editInvoice"
        @download-click="downloadInvoice"
      ></invoice-list>

      <invoice-show :invoice="currentInvoiceShow" :items="items" v-if="showingInvoice"></invoice-show>

      <invoice-edit
        :invoice="currentInvoiceEdit"
        @invoice-saved="savedInvoice"
        @invoice-canceled="cancelEdit"
        v-if="editingInvoice"
      ></invoice-edit>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    showInvoice: function(invoice) {
      ...
      axios.get("api/invoices/" + invoice.id).then(response => {
    ...
    downloadInvoice: function(invoice) {
      console.log("cheguei");
      axios
        .get("api/invoices/pdf/" + invoice.id, {
